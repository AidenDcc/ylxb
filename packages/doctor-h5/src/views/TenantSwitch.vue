    ...
    <div class="tenant-list">
      ...
      >
        <div class="tenant-info">
          ...
          <span class="tenant-type">{{ b.tenantType === 'hospital' ? '🏥' : b.tenantType === 'pharmacy' ? '💊' : '🩺' }} {{ b.role === 'doctor' ? '医生' : '助理' }}</span>
        </div>
        <van-icon v-if="b.tenantId === auth.user?.activeTenantId" name="success" color="#4A90D9" size="20" />
      </div>
    </div>
    <div class="hint">
      <van-notice-bar left-icon="info-o" text="切换服务机构后，患者、咨询、随访等数据将切换至对应机构的专属资源" wrapable />
    </div>
  </div>
</template>
