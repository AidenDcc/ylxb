...
// 快捷入口
const quickActions = [
  { label: '健康档案', icon: Icons.健康档案, path: '/health/records' },
  { label: '健康指标', icon: Icons.健康指标, path: '/health/vital-signs' },
    ...
    <div class="section">
      ...
      <div v-if="activeReminders.length > 0" class="reminder-list">
        <div v-for="r in activeReminders" :key="r.id" class="reminder-item">
          ...
          <van-tag type="primary" size="medium" round>提醒中</van-tag>
        </div>
      </div>
...
/* 用户头部 */
.user-header {
  background: linear-gradient(135deg, #2463EB, #5B8DEF);
  color: #fff;
  border-radius: 12px;
  padding: 20px;
  margin: 16px 0;
...
.more { font-size: 14px; color: #2463EB; cursor: pointer; }
...
.value-num { font-size: 24px; font-weight: 700; color: #333; }
.indicator-card.abnormal .value-num { color: #F5222D; }
.value-unit { font-size: 14px; color: #999; margin-left: 2px; }
.indicator-label { font-size: 14px; color: #666; }
.abnormal-tag {
  position: absolute; top: 6px; right: 6px;
  font-size: 12px; color: #fff; background: #F5222D;
  padding: 1px 6px; border-radius: 8px;
}
.empty-indicator { grid-column: span 2; text-align: center; padding: 20px; color: #ccc; font-size: 14px; }
...
.doctor-name { font-size: 14px; font-weight: 600; color: #333; }
.doctor-title { font-size: 14px; color: #999; }
.unread-badge { font-size: 14px; color: #F5222D; background: #fff2f0; padding: 1px 8px; border-radius: 10px; }
.consultation-preview { font-size: 14px; color: #666; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; margin-bottom: 2px; }
.consultation-time { font-size: 14px; color: #bbb; }
.empty-block { text-align: center; padding: 20px; color: #ccc; font-size: 14px; }
...
.reminder-detail { font-size: 14px; color: #999; margin-top: 2px; }
